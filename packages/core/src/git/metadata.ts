...
/**
 * Get all tracked branches with their metadata.
 * Used for debugging/inspection.
 */
export function getAllBranchMetadata(
  cwd?: string,
): Map<string, BranchMeta | null> {
  const branches = listTrackedBranches(cwd);
  const result = new Map<string, BranchMeta | null>();

  for (const [branchName] of branches) {
    result.set(branchName, readMetadata(branchName, cwd));
  }
