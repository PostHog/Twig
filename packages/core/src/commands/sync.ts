...
export async function sync(options: SyncOptions): Promise<Result<SyncResult>> {
  ...
  // Capture which tracked bookmark we're on BEFORE sync
  // (so we can return to it after the bookmark potentially moves)
  const trackedBookmarks = engine.getTrackedBookmarks();
  let originalTrackedParent: string | null = null;
  const preStatus = await status();
  if (preStatus.ok) {
    const parentBookmarks = preStatus.value.parents[0]?.bookmarks ?? [];
    originalTrackedParent =
      parentBookmarks.find((b) => trackedBookmarks.includes(b)) ?? null;
  }
  ...
  // Handle WC positioning after sync:
  // - If we were on a tracked bookmark before sync, move WC to its NEW tip
  //   (the bookmark may have moved due to new commits from remote)
  // - Otherwise, rebase WC onto trunk
  if (originalTrackedParent) {
    // Move WC to the current tip of the tracked bookmark we were on
    await runJJWithMutableConfigVoid(["new", originalTrackedParent]);
  } else {
    // Not on a tracked bookmark - rebase onto trunk
...
/**
 * Cleanup a single merged/closed change.
 * Reparents children, abandons the change, deletes bookmark, untracks.
 */
export async function cleanupMergedChange(
  change: MergedChange,
  engine: Engine,
): Promise<Result<{ reparentedCount: number; prBasesUpdated: number }>> {
  const { reparentAndCleanup } = await import("../stacks");
  const result = await reparentAndCleanup(change, engine);
  if (!result.ok) return result;
  return ok({
    reparentedCount: result.value.reparentedChildren.length,
