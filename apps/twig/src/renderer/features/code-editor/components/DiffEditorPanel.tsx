...
export function DiffEditorPanel({
  ...
  // Dashboard mode (directRepoPath set) files change frequently due to daemon sync
  const isDashboard = !!directRepoPath;
  const { data: modifiedContent, isLoading: loadingModified } = useQuery({
    ...
    // Dashboard and jj workspace files change frequently, git repo files less so
    staleTime: isJJWorkspace || isDashboard ? 0 : Infinity,
  });
  ...
  const handleContentChange = useCallback(
    async (newContent: string) => {
      ...
      try {
        // Write to preview (main repo)
        // The daemon will automatically sync to the appropriate workspace
