...
export interface AgentSession {
  ...
  events: AcpMessage[];
  startedAt: number;
  status: "connecting" | "connected" | "disconnected" | "error" | "provisioning";
  errorMessage?: string;
...
const useStore = create<SessionStore>()(
  immer((set, get) => {
    ...
    const createNewCloudSession = async (
      ...
      const session = createBaseSession(taskRun.id, taskId, true, effectiveMode);
    ...
    return {
      ...
      actions: {
        connectToTask: async ({
          ...
          const isCloud =
            isCloudOverride ?? latestRun?.environment === "cloud";
